...
async def bot_echo (message: types.Message):
    await message.answer (f"For using our bot, you need to share your phone number first.", reply_markup=phone)
...
@dp.message_handler(text='0',state=Flow.RegisterState)
async def choose_yes(call:types.CallbackQuery):
    await Flow.Pickfruit
    await call.message.answer('pick second season of year',reply_markup=dyn(text_button[3]))
